...
import { createInvoice, sendInvoiceEmail } from "@/app/actions/invoices"
...
import { ArrowLeft, Plus, Trash2, Send, Wrench, Package, FileText } from "lucide-react"
...
export default function NewInvoicePage() {
  ...
  const buildItems = () => {
    ...
    return { labourItems, partsItems, mileageNum, discountNum }
  }
  ...
  const handleSubmit = (intent: "draft" | "send") => {
    ...
    const { labourItems, partsItems, mileageNum, discountNum } = buildItems()
    startTransition(async () => {
      const result = await createInvoice({
        ...
        mileage: mileageNum,
        labour_discount: discountNum,
      })
      if (!result.success || !result.invoiceId) { setError(result.error || "Failed to create invoice"); return }
  ...
  const handleSendWithMessage = (customMessage: string | null) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success || !result.invoiceId) { setError(result.error || "Failed to create invoice"); return }
      await sendInvoiceEmail(result.invoiceId, customMessage)
      router.push(`/admin/invoices/${result.invoiceId}`)
    })
  }
                    ...
                    <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
                      <div className="col-span-5">
                        <Input
                          ...
                          onChange={e => updateLabour(row.id, "description", e.target.value)}
                  ...
                  return (
                    <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
                      <div className="col-span-5">
                        <Input
                          value={row.description}
                          onChange={e => updateParts(row.id, "description", e.target.value)}
                      ...
                      <div className="col-span-1 flex justify-center">
                        <button
                          type="button"
                          onClick={() => removeParts(row.id)}
                          disabled={partsRows.length === 1}
                          className="text-gray-600 hover:text-red-400 disabled:opacity-20 transition-colors"
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
