...
import { updateInvoice, getInvoice, sendInvoiceEmail } from "@/app/actions/invoices"
...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    ...
    getInvoice(invoiceId).then(inv => {
      if (!inv) { router.push("/admin/invoices"); return }

      const dateStr = inv.invoice_date || inv.created_at
      setInvoiceDate(dateStr ? dateStr.slice(0, 10) : new Date().toISOString().slice(0, 10))
      ...
      setNotes(inv.notes ?? "")
      setMileage(inv.mileage ? String(inv.mileage) : "")
      setLabourDiscount(inv.labour_discount ? String(inv.labour_discount) : "")

      try {
        const labour = JSON.parse(inv.labour_items || "[]") as Array<{description:string;hours:number;rate:number}>
      ...
      try {
        const parts = JSON.parse(inv.parts_items || "[]") as Array<{description:string;qty:number;unitPrice:number}>
  ...
  const discountPct = parseFloat(labourDiscount) || 0
  ...
  const buildUpdateData = () => {
    ...
    const mileageNum = mileage.trim() ? parseInt(mileage.trim(), 10) || null : null
    const discountNum = labourDiscount.trim() ? parseFloat(labourDiscount.trim()) || null : null
    return {
      ...
      vat_enabled: vatEnabled ? 1 : 0, vat_rate: vatRate, invoice_date: invoiceDate || null,
      mileage: mileageNum, labour_discount: discountNum,
    }
  }
  ...
  const handleSubmit = (intent: "save" | "send") => {
    ...
    startTransition(async () => {
      const result = await updateInvoice(invoiceId, buildUpdateData())
      if (!result.success) { setError(result.error || "Failed to save invoice"); return }
  ...
  const handleSendWithMessage = (customMessage: string | null) => {
    ...
    startTransition(async () => {
      const result = await updateInvoice(invoiceId, buildUpdateData())
      if (!result.success) { setError(result.error || "Failed to save invoice"); return }
      await sendInvoiceEmail(invoiceId, customMessage)
      router.push(`/admin/invoices/${invoiceId}`)
    })
  }
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-gray-300">Invoice Date <span className="text-red-400">*</span></Label>
                  <Input
                    type="date"
                    value={invoiceDate}
                ...
                <div className="space-y-2">
                  <Label className="text-gray-300">Mileage at Service</Label>
                  <Input
                    inputMode="numeric"
                    value={mileage}
                  ...
                  return (
                    <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <div className="col-span-2">
                        <Input inputMode="decimal" value={row.hours} onChange={e => updateLabour(row.id,"hours",e.target.value)}
                          placeholder="1.5"
                          className="bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500 text-sm text-right focus:border-blue-500" />
                      </div>
                      <div className="col-span-2">
                        <Input inputMode="decimal" value={row.rate} onChange={e => updateLabour(row.id,"rate",e.target.value)}
                          placeholder="60.00"
                          className="bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500 text-sm text-right focus:border-blue-500" />
                      </div>
                ...
                {partsRows.map(row => {
                  ...
                  return (
                    <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <div className="col-span-2">
                        <Input inputMode="numeric" value={row.qty} onChange={e => updateParts(row.id,"qty",e.target.value)}
                          placeholder="1"
                          className="bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500 text-sm text-right focus:border-blue-500" />
                      </div>
                      <div className="col-span-2">
                        <Input inputMode="decimal" value={row.unitPrice} onChange={e => updateParts(row.id,"unitPrice",e.target.value)}
                          placeholder="25.00"
                          className="bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500 text-sm text-right focus:border-blue-500" />
                      </div>
                      <div className="col-span-2 text-right text-gray-300 text-sm font-medium pr-1">
          ...
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-gray-300">Labour Discount (%)</Label>
                <Input
                  inputMode="decimal"
                  value={labourDiscount}
                  onChange={e => setLabourDiscount(e.target.value)}
                  placeholder="e.g. 10 (parts are not discounted)"
                  className="bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500 focus:border-blue-500 max-w-xs"
                />
              </div>
              <div className="space-y-2">
