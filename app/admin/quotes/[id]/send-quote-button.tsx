...
export default function SendQuoteButton({
  quoteId, quoteNumber, customerName, vehicle, hasSumUp, existingPaymentLink, existingDepositAmount
}: Props) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [mode, setMode] = useState<"plain" | "deposit" | null>(null)
  const [depositAmt, setDepositAmt] = useState(existingDepositAmount ? existingDepositAmount.toFixed(2) : "")
  ...
  async function handleSend() {
    ...
    try {
      if (mode === "deposit") {
        ...
        const desc = `Parts deposit for ${vehicle || customerName} — Quote ${quoteNumber}`
        const checkout = await createSumUpCheckout(quoteId, amt, desc, "quote")
      ...
      const result = await sendQuoteEmailAction(quoteId, null, link, deposit)
      if (!result.success) {
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-50 p-3 space-y-2">
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-0.5">Send quote PDF — no payment link</p>
          </button>

          {/* Option 2: deposit link */}
          {hasSumUp && (
            <button
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-0.5 ml-5">Customer pays deposit before you order parts</p>
            </button>
          )}
          ...
          {payLink && mode === "plain" && (
            <div className="px-2 py-2 bg-green-900/30 border border-green-700 rounded-lg">
              <p className="text-xs text-green-300 font-medium">Existing payment link:</p>
